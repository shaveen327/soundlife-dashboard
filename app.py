...
- Dropout is not fully random — it is related to observable characteristics (CMV status, sex, age group). The model should not assume missing later visits are a random subset.
        ...
        warning("CMV-positive young women had notably fewer study visits (mean 7.2) "
                "and the lowest completion rate. Dropout is informative, not random.")
...
- The Oncology panel has more proteins with high LOD rates, as many tumour markers are undetectable in healthy subjects.
        ...
        st.plotly_chart(fig2, use_container_width=True)
        warning("217 proteins are below LOD in >30% of samples. A group decision is needed: "
                "exclude these proteins, or model them as censored (value known only to be below "
                "a threshold) rather than treating the recorded NPX number as a true measurement.")
